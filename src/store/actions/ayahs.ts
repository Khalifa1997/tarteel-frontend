...
export const loadNextAyah = (currentAyah?: AyahShape) => {
  return async (dispatch, getState: () => ReduxState) => {
    ...
    const {verseNumber: ayah, chapterId: surah} = currentAyah || getState().ayahs.currentAyah;
...
export const loadNextQueue = () => {
  return (dispatch, getState: () => ReduxState) => {
    return new Promise(async (resolve) => {
      ...
      if (getState().ayahs.nextAyah[0].verseNumber === getState().ayahs.currentAyah.verseNumber) {
...
export const loadPrevQueue = () => {
  return (dispatch, getState: () => ReduxState) => {
    return new Promise(async (resolve) => {
      ...
      if (getState().ayahs.prevAyah[0].verseNumber === getState().ayahs.currentAyah.verseNumber) {
