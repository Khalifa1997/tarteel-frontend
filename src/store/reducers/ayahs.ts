...
export default (
  ...
  switch (action.type) {
    ...
    case LOAD_NEXT_AYAH_SUCCESS:
      return {
        ...state,
        nextAyah: uniqBy([...state.nextAyah, action.payload], 'verseNumber'),
      };
    case getType(ayahs.unShiftNextAyah):
      return {
        ...state,
        nextAyah: uniqBy([action.payload, ...state.nextAyah], 'verseNumber'),
      };
    ...
    case LOAD_PREV_AYAH_SUCCESS:
      return {
        ...state,
        prevAyah: uniqBy([...state.prevAyah, action.payload], 'verseNumber'),
      };
    case getType(ayahs.unShiftPrevAyah):
      return {
        ...state,
        prevAyah: uniqBy([action.payload, ...state.prevAyah], 'verseNumber'),
      };
